...
// Visible/Continuingpool/Placementpool/Played/Isfollower are IntBool:
// always known even though placementpool is a nullable column, so no pointer needed
//
// Drawsallowed..Follower come from games, and are absent for a pool with no games.
type Pool struct {
	PoolID           int64
	Name             string
	Ordering         string
