...
internal/livedatamodel/v01_09_14/  the Live! 1.9.14 API's response shapes
...
internal/livepublish/v01_09_14/    renders a data/<slug>.db as static JSON in docs/
...
Package names (and `-version` flag values) are the Live! app version with each dot-separated segment zero-padded to two digits and joined by underscores (`1.9.14` -> `v01_09_14`).
...
| 1.9.17                    | v01_09_17       | In Development |
| 2.0.0                     | -               | Not Planned    |
...
2. **Publish** the archive to `docs/` for GitHub Pages, via `-mode publish` or `-mode all` (planned; each version's `livepublish` package currently just returns "not yet implemented").
